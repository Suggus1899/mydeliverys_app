...
*   **Protección de Recursos y Rate Limiting:** Ninguna petición entrante puede sobrecargar la base de datos PostgreSQL. Toda ruta pública o de alta frecuencia debe implementar cuotas estrictas de rate limiting por usuario e IP en Redis. Peticiones que excedan el límite deben responder con `HTTP 429` estandarizado antes de tocar la base de datos.
*   **Fidelidad al Sistema de Diseño (Design System):** Ningún desarrollador de Flutter puede inventar estilos, colores o tipografías ad-hoc. Toda la interfaz debe regirse exclusivamente por los tokens definidos en `docs/sdd/design_system.md` (colores de marca, semáforo de estados, tipografía Outfit/Inter con números tabulares y componentes adaptados al sol llanero).
